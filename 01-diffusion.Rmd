...
3. Conditioning on a single data point $z$ with dirac delta target $\delta_z$, the conditional vector field has simple analytic solution. Apply the marginalization trick \@ref(thm:margTrick) to generalize to whole empirical data distribution. 
    - <u>Very important remark</u>: fixing $\Pinit$, the mapping $u_t\mapsto p_t$ **is not linear**. The vector field $u_t$ specifies the **probability velocity**, while mixture linearly mixes the **probability flux** $u_tp_t$.
...
5. <span style="blue"> Key leverage via deep learning </span>: having solved the simple conditional case analytically, we're relying on the model to: 
...
7. Classifier free guidance \@ref(def:cfg) contrastively amplifies the guided v. unguided vector fields. Under Gaussian path, this has the nice interpretation of manually upweighing the score contribution from classification log-likelihood. See detailed training procedure \@ref(def:cfgProtocol). 
...
_Proof:_ Expanding the squared norm for both losses, it suffices to show that 
\[ 
...
\begin{aligned}
    ...
    \right\|
\end{aligned}
For simple linear interpolation, this instantiates to $\mca L_\CFM(\theta) = \Exp \|u_t^\theta(tz + \bar t\epsilon) - (z-\epsilon)\|^2$. In terms of score, we obtain 
\begin{aligned}
    \mca L_\CSM(\theta)
...
We would further like to be able to condition generation on prompts / features / past data $y\in \mca Y$. The generation process becomes 
\[ 
    Y\xrightarrow{\Pdata(z\mid y)} Z\xrightarrow{p_t(x\mid z)} X
\] 

:::{.definition name="guided diffusion model"}
A guided diffusion (flow) model consists of a guided vector field $u_t^\theta(\cdot \mid y)$ 
which accepts $t, y\in \mca Y$ to output change in $X$. Essentially we're trying to jointly model $\Pdata(z, y)$. 
:::

Fixing $y$, the conditional flow matching loss writes 
\[ 
    \mca L_{\CFM, y}(\theta) = \Exp_{t\sim \Unif, z\sim \Pdata(\cdot\mid y), x\sim p_t(\cdot\mid z)} \left\|u_t^\theta(x) - u_t^\Target(x\mid z)\right\|^2 
\] 
Take expectation over $y$ to obtain the guided conditional loss 
\[ 
    \mca L_\CFM^\Guided(\theta) = \Exp_{t\sim \Unif, (z, y)\sim \Pdata(z, y), x\sim p_t(\cdot\mid z)} \|u_t^\theta(x\mid y) - u_t^\Target(x\mid z) \|^2
\] 
Empirically, people have observed that the loss above does not enforce guidance strongly enough. Recalling the linear Gaussian formula \@ref(prp:gaussianFormulae), we can rewrite 
\[ 
    u_t^\Target(x\mid y) = a_t x + b_t \nabla_x \log p_t(x\mid y), \quad (a_t, b_t) = \left(\df{\dot \alpha_t}{\alpha_t}, \df{\dot \alpha_t \beta_t^2 - \dot \beta_t \beta_t \alpha_t}{\alpha_t}\right) 
\] 
Rewrite guided score as $\nabla \log p_t(x\mid y) = \nabla \log p_t(y\mid x) + \nabla \log p_t(x)$: 
- Guided score at noisily guided data $x_t$ is equivalent to the un-guided score of $x_t$, plus log-likelihood of the guidance label $y$, given $x_t$. 

To artificially amplify guidance, consider introducing the **guidance scale** $w>1$ and rearrange
\begin{aligned}
    \tilde u_t(x\mid y) 
    &= a_t x + b_t \left(
    ...
    &= a_t x + b_t(1-w) \nabla \log p_t(x) + w \nabla \log p_t(x\mid y) \\ 
    &= w\, u^\Target_t(x\mid y) + \bar w\, u^\Target_t(x) 
\end{aligned}
So for Gaussian conditional probability path, <span style="color:blue">up-scaling guidance from classifier = linearly interpolating between $u_t^\Target(x\mid y)$ and $u_t^\Target(x)$.$</span>. 

:::{.definition #cfg name="classifier-free guidance"}
Per the discussion above, the **classifier-free guided** vector field and scores are defined, given guidance scale $w>1$, as 
\begin{aligned}
    \tilde u_t(x\mid y) 
    &= w\, u^\Target_t(x\mid y) + \bar w\, u^\Target_t(x), \quad 
    \tilde s_t(x\mid y) 
    = w\, s^\Target_t(x\mid y) + \bar w\, s^\Target_t(x) 
\end{aligned}
:::

:::{.definition #cfgProtocol name="training protocol for linear Gaussian path"}
Given label-data pairs $(y_j, z_j)$, guidance scale $w>1$, and probability $\eta\in (0, 1)$, the training procedure for CFG-flow matching with linear Gaussian probability path \@ref(def:condGaussianPath) is as follows:
...
3. Sample $t\sim \Unif, \epsilon \sim \mca N(0, I_d)$ batch-wise. 
4. Minimize flow matching loss: 
\begin{aligned}
    \mca L^{\mrm{CFG}}_\CSM 
    = \df 1 n \sum_{j=1}^n \left\|
        u_{t_j}^\theta(t_jz_j + \bar t_j\epsilon_j\mid y_j) - (z_j-\epsilon_j)
    \right\|^2
\end{aligned}

This trains the model to match flow of $u_t^\Target(x\mid y)$ for conditional case, and $u_t^\Target(x\mid \emptyset)$ for unconditional case. Guided sampling given $y$: 

1. Initialize $x_0\sim \Pinit$. 
2. Simulate ODE $dx_t = \left[ w\, u^\theta_t(x_t\mid y)  + \bar w\, u^\theta_t(x_t\mid \emptyset)
\right]\, dt$ 
3. Output $x_1$. 
:::
